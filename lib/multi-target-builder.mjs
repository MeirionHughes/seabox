...
export class MultiTargetBuilder {
  ...
  async buildTarget(outputConfig, bundledEntryPath, nativeModules, detectedAssets, buildNumber) {
    ...
    if (this.verbose) {
      console.log(`\n   📋 Embedded Assets (${allAssets.length} total):`);
      ...
      if (nativeAssets.length > 0) {
        console.log(`   \n   Native Modules (${nativeAssets.length}):`);
        for (const asset of nativeAssets) {
          const sizeMB = (fs.statSync(asset.sourcePath).size / 1024 / 1024).toFixed(2);
          // Show just the filename for cleaner output
          const displayName = path.basename(asset.assetKey);
          console.log(`     - ${displayName} (${sizeMB} MB)`);
        }
      }
      
      if (libraryAssets.length > 0) {
        console.log(`\n   Platform Libraries (${libraryAssets.length}):`);
        for (const asset of libraryAssets) {
          const sizeMB = (fs.statSync(asset.sourcePath).size / 1024 / 1024).toFixed(2);
          console.log(`     - ${asset.assetKey} (${sizeMB} MB)`);
        }
      }
      
      if (regularAssets.length > 0) {
        console.log(`\n   Regular Assets (${regularAssets.length}):`);
        for (const asset of regularAssets) {
          const sizeKB = (fs.statSync(asset.sourcePath).size / 1024).toFixed(2);
          console.log(`     - ${asset.assetKey} (${sizeKB} KB)`);
        }
      }
      console.log('');
    }
    ...
    for (const [moduleKey, moduleInfo] of nativeModules) {
      ...
      try {
        ...
        if (moduleInfo.isPrebuild) {
          if (this.verbose) {
            console.log(`      ✓ Using prebuild: ${moduleName}`);
          }
